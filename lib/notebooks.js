...
async function fetchUserDementors(signer) {
  ...
  const userDementors = await contract.getUserDementors();
  const dementorsDataArray = [];
  for (let i = 0; i < userDementors.length; i++) {
    const dementorId = ethers.utils.formatUnits(userDementors[i], 0);

    const dementorData = await contract.getDementor(dementorId);
...
async function processFetchedDementor(rawDementor) {
  try {
    ...
    let modifiedMetadataURI = rawDementor.promptCID;
    ...
    const thisPagePrompts = await metadataUrlToObject(modifiedMetadataURI);
    console.log('the metadata for this dementor is: ', thisPagePrompts);
    const dementor = JSON.parse(thisPagePrompts);
    return dementor;
  } catch (error) {
...
async function processFetchedNotebook(rawNotebook) {
  try {
    ...
    const processedNotebook = {
      ...
      templateId: Number(ethers.utils.formatUnits(rawNotebook.templateId, 0)),
...
async function formatUserJournal(rawJournal) {
  ...
  return {
    ...
    journalId: Number(ethers.utils.formatUnits(rawJournal[2], 0)),
...
function formatUserJournals(rawJournals) {
  return rawJournals.map(journal => {
    ...
    return {
      ...
      pagesLeft: journal[1],
      journalId: ethers.utils.formatUnits(journal[2], 0),

...
async function fetchUserNotebooks(signer, userTba, thisWallet) {
  ...
  const notebookIds = await contract.getUserNotebooks();
...
async function fetchUserEulogias(signer, thisWallet) {
  ...
  const userCreatedEulogias = await contract.getUserCreatedEulogias();
  const eulogiaDataArray = [];
  for (let i = 0; i < userCreatedEulogias.length; i++) {
    const thisEulogiaId = userCreatedEulogias[i];

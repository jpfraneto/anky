...
async function processFetchedEulogia(rawEulogia) {
  console.log('IN HERES', rawEulogia);
  try {
    // Check and modify the metadataURI if it starts with "https://arweave.net/"
    let modifiedMetadataURI = rawEulogia.metadataURI;
    const arweavePrefix = 'https://arweave.net/';
    ...
    const eulogia = {
      metadata,
      maxMessages: Number(ethers.utils.formatUnits(rawEulogia.maxMessages, 0)),
      messageCount: ethers.utils.formatUnits(rawEulogia.messageCount, 0),
    };

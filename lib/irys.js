...
async function getDementorInfoFromIrys(
  ...
  unprocessedPages.forEach(page => {
    ...
    if (page.dementorAnswer) {
      newDementor.pages[Number(page.pageNumber)].writings =
        page.text.split('---');
      newDementor.pages[Number(page.pageNumber)].answerTimestamp =
        page.timestamp;
      newDementor.pages[Number(page.pageNumber)].answerCid = page.cid;
    } else {
      if (newDementor.pages.length == page.pageNumber) {
        console.log('INSAIJIDHASOUIA', page.text);
        let newPage = {
          prompts: page.text.split('%%'),
          promptsTimestamp: page.timestamp,
          promptsCid: page.cid,
        };
        newDementor.pages.push(newPage);
      } else {
        newDementor.pages[Number(page.pageNumber)].prompts = page.text;
        newDementor.pages[Number(page.pageNumber)].promptsTimestamp =
          page.timestamp;
        newDementor.pages[Number(page.pageNumber)].promptsCid = page.cid;
      }
    }
  });
  console.log('the new dementor is: ', newDementor);
